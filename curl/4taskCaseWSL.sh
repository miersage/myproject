...
curl.exe -X POST -H 'Content-Type: application/json' -i "http://$ip/api/task" \
...
curl.exe -X GET -i "http://$ip/api/task" >>$out
echo -e "\n" >>$out

# Test Read task 3
echo -e "--Test Read task 3\n" >>$out
curl.exe -X GET -i "http://$ip/api/task/3" >>$out
echo -e "\n" >>$out

# Test List tasks by account 2
echo -e "--Test List tasks by account 2\n" >>$out
curl.exe -X GET -i "http://$ip/api/task1/2" >>$out
echo -e "\n" >>$out

# Test List tasks by account 2, taskList 2
echo -e "--Test List tasks by account 2, tasklist 2\n" >>$out
curl.exe -X GET -i "http://$ip/api/task1/2/2" >>$out
echo -e "\n" >>$out

# Test Modify task 4
echo -e "--Test Modify task 4\n" >>$out
curl.exe -X PUT -H 'Content-Type: application/json' -i "http://$ip/api/task/4" \
...
curl.exe -X DELETE -i "http://$ip/api/task/4" >>$out
echo -e "\n" >>$out
echo -e "--Check List of tasks\n" >>$out
curl.exe -X GET -i "http://$ip/api/task" >>$out
echo -e "\n" >>$out
