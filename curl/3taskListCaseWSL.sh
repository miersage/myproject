...
curl.exe -X POST -H 'Content-Type: application/json' -i "http://$ip/api/tasklist" \
...
curl.exe -X GET -i "http://$ip/api/tasklist" >>$out
echo -e "\n" >>$out

# Test Read tasklist 3
echo -e "--Test Read tasklist 3\n" >>$out
curl.exe -X GET -i "http://$ip/api/tasklist/3" >>$out
echo -e "\n" >>$out

# Test List tasklists by account 2
echo -e "--Test List tasklists by account 2\n" >>$out
curl.exe -X GET -i "http://$ip/api/tasklist1/2" >>$out
echo -e "\n" >>$out

# Test Modify tasklist 4
echo -e "--Test Modify tasklist 4\n" >>$out
curl.exe -X PUT -H 'Content-Type: application/json' -i "http://$ip/api/tasklist/4" \
...
curl.exe -X DELETE -i "http://$ip/api/tasklist/4" >>$out
echo -e "\n" >>$out
echo -e "--Check List of tasklists\n" >>$out
curl.exe -X GET -i "http://$ip/api/tasklist" >>$out
echo -e "\n" >>$out
